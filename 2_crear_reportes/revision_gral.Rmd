...
copiaRenombra(base_sqlite[1], dir_padre, entrega)
copiaRenombra(base_csv[1], dir_padre, entrega)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Conexión a la base de datos
base <- paste(dir_padre, "/", entrega, ".sqlite", sep = "")

base_input <- src_sqlite(base)

# Si la base contiene conglomerados repetidos cambiamos el valor de nombre por
# nombre-aa donde aa indica el año (ej: 72372 -> 72372-14) para poder 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Revisamos si hay conglomerados que ya estén en la base de datos
# Revisamos la imagen de la base de datos postgresql
dir_imagen <- "../6_fusionar_postgres/imagen"
file_imagen <- list.files(path = dir_imagen, recursive = TRUE, 
  full.names = TRUE, pattern = "\\.sqlite")
flag_imagen <- FALSE
...
if(length(file_imagen) > 0){
  ...
  tab_imagen <- base_imagen %>%
    ...
    select(conglomerado_muestra_id = id, cgl = nombre, fecha = fecha_visita, 
      edo. = estado, mun. = municipio) %>%
    mutate(base = "final")
  tab_cgl_comp <- cgl_1 %>%
    filter(cgl %in% tab_imagen$cgl) %>%
    select(conglomerado_muestra_id, cgl, fecha, edo. = estado, 
      mun. = municipio) %>%
    mutate(base = "entrega")
  tab_imagen_fecha_aux <- tab_imagen %>%
    rbind(tab_cgl_comp) %>%
    arrange(cgl, desc(base))
  tab_imagen_fecha <- tab_imagen_fecha_aux %>%
    select(-conglomerado_muestra_id)
  
  # creamos una lista de ids con los conglomerados que ya existen en la 
  # base de datos, esto para eliminarlos antes de fusionar
  ids_reps_final <- tab_imagen_fecha_aux$conglomerado_muestra_id[
    duplicated(select(tab_imagen_fecha_aux, cgl, fecha))]
  
  if(length(ids_reps_final) > 0){
    output_txt = paste(output_dir, "/", entrega, "_rep_final.txt", sep = "")
    write.table(ids_reps_final, file = output_txt, quote = FALSE, 
