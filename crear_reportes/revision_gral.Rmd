...
tab_camara <- archivos %>%
  ...
  summarise(
    ...
    n = sum(!is.na(id)),
    fotos = round(100 * (sum(tipo == "JPG") / n)),
    videos = round(100 * (sum(tipo == "AVI") / n)),
    fauna = round(100 * (sum(presencia == "T", na.rm = TRUE) / n)),
    sin_fauna = round(100 * (sum(presencia == "F", na.rm = TRUE) / n))
    ) %>%
...
tab_grabadora <- archivos %>%
  ...
  summarise(
    ...
    fecha_termino = paste(fecha_termino, hora_termino),
    n = sum(!is.na(id)),
    audibles = round(100 * (sum(audible, na.rm = TRUE) / n)),
    ultrasonicos = round(100 * (sum(ultra, na.rm = TRUE) / n))
    ) %>%
  select(-hora_inicio, -hora_termino)
...
tab_epifitas <- collect(tbl(base_input, "Informacion_epifitas")) %>%
  left_join(sitio, by = c("sitio_muestra_id" = "id")) %>%
  right_join(conglomerado, by = c("conglomerado_muestra_id" = "id")) %>%
  select(nombre, epif_b = helechos_observados)
